...
const AdminDashboard = () => {
  ...
  const [newUser, setNewUser] = useState({ name: '', email: '', password: 'password123', role: 'student', registrationNumber: '', branch: 'CSE', semester: 6 });
  const [newCourse, setNewCourse] = useState({ courseCode: '', name: '', faculty: '', type: 'theory', maxClasses: '', branch: 'CSE', semester: 6 });
  ...
  const handleCreateUser = async (e) => {
    ...
    try {
      ...
      setNewUser({ name: '', email: '', password: 'password123', role: 'student', registrationNumber: '', branch: 'CSE', semester: 6 });
      fetchData();
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      ...
      setNewCourse({ courseCode: '', name: '', faculty: '', type: 'theory', maxClasses: '', branch: 'CSE', semester: 6 });
      fetchData();
  ...
  const studentColumns = useMemo(() => [
    ...
    { accessorKey: 'branch', header: 'Branch', cell: info => <span className="badge badge-primary">{info.getValue() || 'CSE'}</span> },
    { accessorKey: 'semester', header: 'Semester', cell: info => <span className="badge badge-neutral">Sem {info.getValue() || 6}</span> },
  ...
  const coursesColumns = useMemo(() => [
    ...
    { accessorKey: 'branch', header: 'Branch', cell: info => <span className="badge badge-primary">{info.getValue() || 'CSE'}</span> },
    { accessorKey: 'semester', header: 'Sem', cell: info => <span className="badge badge-neutral">{info.getValue() || 6}</span> },
  ...
  const TabButton = ({ id, label, icon }) => (
    <button
      style={{
        ...
        borderBottom: `2px solid ${tab === id ? 'var(--primary-500)' : 'transparent'}`,
        color: tab === id ? 'var(--primary-600)' : 'var(--text-muted)',
  ...
  const barChartData = [
    { name: 'CS101', value: 92 }, { name: 'CS102', value: 85 }, { name: 'IT201', value: 74 },
    { name: 'EE101', value: 89 }, { name: 'MECH1', value: 68 }
  ];
  ...
  return (
    <Layout navItems={navItems} pageTitle="Administration Hub">
      ...
      {loading ? (
        ...
        <motion.div
          className="kpi-grid"
          style={{ gridTemplateColumns: 'minmax(280px, 2fr) minmax(200px, 1fr) minmax(200px, 1fr)', marginBottom: 'var(--space-4)' }}
          initial="hidden" animate="visible"
          variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }}
        >
          <KPICard title="Global Cohort Health Rate" value="88.4%" icon={<TrendingUp />} trend="up" trendValue="2.1%" metaText="vs last semester" isPrimary={true} />
          <KPICard title="Active Students" value={studentsList.length} icon={<UsersIcon />} trend="neutral" trendValue="0%" metaText="Stable enrollment" />
          <KPICard title="Flagged Students" value={atRiskTableData.length} icon={<AlertCircle />} trend="down" trendValue="12%" metaText="Drop from midterms" />
        </motion.div>
      )}
              ...
              <form onSubmit={handleCreateUser}>
                <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  ...
                  <div className="form-group">
                    ...
                    <select className="input-sys" value={newUser.branch} onChange={e => setNewUser({ ...newUser, branch: e.target.value })}>
                  ...
                  {newUser.role === 'student' && (
                    <div className="form-group">
                      ...
                      <input className="input-sys" type="number" min="1" max="8" value={newUser.semester} onChange={e => setNewUser({ ...newUser, semester: e.target.value })} />
                    </div>
                  )}
                </div>
              ...
              <form onSubmit={async (e) => {
                ...
                <div className="form-group">
                  ...
                  <p style={{ fontSize: '13px', color: 'var(--text-secondary)', marginBottom: '8px' }}>
                    CSV must include headers: <strong>name, registrationNumber</strong>. Optional: <strong>branch, semester</strong>. Default email mapping applied automatically if omitted.
                  </p>
            ...
            <div style={{ padding: '24px' }}>
              <form onSubmit={handleCreateCourse}>
                ...
                <div className="form-grid-3" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                  <div className="form-group">
                    <label className="form-label">Faculty Linkage</label>
                  ...
                  <div className="form-group">
                    <label className="form-label">Pedagogy Mode</label>
                ...
                <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginTop: '16px' }}>
                  <div className="form-group">
                    <label className="form-label">Target Branch</label>
                    <select className="input-sys" value={newCourse.branch} onChange={e => setNewCourse({ ...newCourse, branch: e.target.value })}>
                      <option value="CSE">CSE</option>
                      <option value="ECE">ECE</option>
                      <option value="ME">Mechanical</option>
                      <option value="CE">Civil</option>
                  ...
                  <div className="form-group">
                    ...
                    <input className="input-sys" type="number" min="1" max="8" value={newCourse.semester} onChange={e => setNewCourse({ ...newCourse, semester: e.target.value })} />
                  </div>
                </div>
                <div style={{ marginTop: '16px', display: 'flex', justifyContent: 'flex-end' }}>
            ...
            <div style={{ padding: '24px' }}>
              <form onSubmit={handleEnroll}>
                <div className="form-grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                  ...
                  <div className="form-group">
                    <label className="form-label">Destination Node (Course)</label>
      ...
      <SideDrawer isOpen={drawerOpen} onClose={() => setDrawerOpen(false)} title="System Details Inspector">
        {drawerData && drawerType === 'student' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ padding: '16px', background: 'var(--bg-surface-hover)', borderRadius: 'var(--radius-lg)' }}>
              ...
              {drawerData.enrolledCount === 0 ? (
                <p style={{ fontSize: '13px', color: 'var(--text-secondary)' }}>No vectors established.</p>
              ) : (
        ...
        {drawerData && drawerType === 'atrisk' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <button className="btn btn-primary" style={{ width: '100%', marginTop: 'auto', background: 'var(--red-600)', borderColor: 'var(--red-600)' }}
              onClick={() => { flash('Intervention protocol active. Alert sent.', 'success'); setDrawerOpen(false); }}>
